...
/**
 * @brief Calculate the cross power spectrum of two grids A and B, defined by
 * P_AB(k) * (2pi)^3 * delta^3(k+k') = <A(k)B(k')^*>.
 *
 ...
 */
void calc_cross_powerspec(int width, double box_len, fftw_complex *k_box1, fftw_complex *k_box2, int bins, double *k_in_bins, double *power_in_bins, int *obs_in_bins, int deconvolve) {
       ...
       for (int y=0; y<width; y++) {
           for (int z=0; z<=half_width; z++) {
               ...
               if (k>0) {
                   ...
                   if (bin >= 0 && bin < bins) {
                       double a1 = k_box1[row_major_id_half_periodic(x, y, z, N)][0];
                       double b1 = k_box1[row_major_id_half_periodic(x, y, z, N)][1];

                       double a2 = k_box2[row_major_id_half_periodic(x, y, z, N)][0];
                       double b2 = k_box2[row_major_id_half_periodic(x, y, z, N)][1];
                       ...
                       if (deconvolve == CLOUD_IN_CELL) {
                           ...
                           power /= WW;
                       } else if (deconvolve == TRIANGULAR_CLOUD) {
                           //The TSC Window function in Fourier space
                           double W_x = (k_x == 0) ? 1 : pow(sinc(0.5*k_x*box_len/N),3);
                           double W_y = (k_y == 0) ? 1 : pow(sinc(0.5*k_y*box_len/N),3);
                           double W_z = (k_z == 0) ? 1 : pow(sinc(0.5*k_z*box_len/N),3);
                           double W = W_x*W_y*W_z;
                           double WW = W*W;
                       ...
                       //Should not happen (incorrect bins in power spectrum calculation)
                       error("Icorrect bins in power spectrum calculation");
                   }
               }
           }
       }
