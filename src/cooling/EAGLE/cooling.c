...
INLINE static double bisection_iter(
  ...
  double LambdaNet_cgs =
      ...
      eagle_cooling_rate(log10(u_ini_cgs), redshift, n_H_cgs, abundance_ratio,
                         n_H_index, d_n_H, He_index, d_He, cooling, recompute_cooling_rate_flag);
...
// ALEXEI: temporarily change return type to double for debugging
double cooling_cool_part(const struct phys_const *phys_const,
  ...
  //if (dt == 0.) return;
  // ALEXEI: return initial energy, used for debugging
  if (dt == 0.) return hydro_get_physical_internal_energy(p, xp, cosmo);
  ...
  // ALEXEI: return final energy for debugging
  return u_final;
}
