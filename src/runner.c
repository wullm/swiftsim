...
#ifdef SWIFT_DEBUG_CHECKS
	    if(sp != &c->stars.parts[0])
	      error("Newly created star not at the correct position.");
#endif
...
#ifdef SWIFT_DEBUG_CHECKS
	    if(gp != temp_ptr)
	      error("Received the wrong naked gpart");
#endif
