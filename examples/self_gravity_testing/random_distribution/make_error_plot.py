...
plt.figure()
ax1 = plt.subplot(211)
plt.title("Orbiting random distribution of 10 particles at 3 AU")
...
in_file = "./data/orbit_r_%d_mle_%d_m_5_theta_0.dat" %(int(np.rint(r)),mle) 
...
for m in m_vals:
    ...
    ### read in positions
    in_file = "./data/orbit_r_%d_mle_%d_m_%d_theta_1.dat" %(int(np.rint(r)),mle,m) 
    a = np.genfromtxt(in_file)
    t = a[:,0]
    x_multipole = a[:,1]
    y_multipole = a[:,2]
    ...
    in_file = "./data/E_L_r_%d_mle_%d_m_%d_theta_1.dat" %(int(np.rint(r)),mle,m)
    a = np.genfromtxt(in_file)
    t = a[:,0]
    E = a[:,1]
    L = a[:,2]
...
#plt.show()
plt.savefig("./plots/random_multipole_test_r_%d.png" %int(np.rint(r)) , format = "png")
plt.close()
