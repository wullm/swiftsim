...
float powerSpectrum(float k) {
	return pow(k, n_s); //normalization is fixed by sigma_8 in main code, so A_s/k_star are irrelevant
}
...
const float damping_scale = 41.0/mass_neutrino; //Mpc - see eq. (2) in Frenk, White, and Davis (1983)

//Numerical constant [(2/3)*(pi^2/zeta(3))]^(1/3) appearing in neutrino temperature-density relation
...
const double b_1				= 0.313*pow(Omega_m*h*h, -0.419)*(1+0.607*pow(Omega_m*h*h, 0.674));
const double b_2				= 0.238*pow(Omega_m*h*h, 0.223);
const double z_d				= 1291*pow(Omega_m*h*h, 0.251)/(1+0.659*pow(Omega_m*h*h, 0.828))*(1+b_1*pow(Omega_b*h*h, b_2));

//Calculate the sound horizon at the drag epoch
const double R_0				= 31.5*Omega_b*h*h*pow(Theta_2p7, -4); //dimensionless
double R_bpm(double z) { //The baryon to photon momentum ratio
	return R_0/(z/1000);
}
const double R_eq				= R_bpm(z_eq);
const double R_d				= R_bpm(z_d);
const double s					= (2.0/(3*k_eq))*sqrt(6.0/R_eq)*log((sqrt(1+R_d)+sqrt(R_d+R_eq))/(1+sqrt(R_eq))); //Mpc

//Calculate the silk damping scale
const double k_silk				= 1.6*pow(Omega_b*h*h, 0.52)*pow(Omega_m*h*h, 0.73)*(1+pow(10.4*Omega_m*h*h, -0.95)); //Mpc^-1
...
double T_0_tilde(double k, double alpha_c, double beta_c) {
	...
	double output		= log(M_E+1.8*beta_c*q)/(log(M_E+1.8*beta_c*q)+C*q*q);
	return output;
}
...
double Transfer(double k) {
	...
	double a_1			= pow(46.9*Omega_m*h*h, 0.670)*(1+pow(32.1*Omega_m*h*h, -0.532));
	double a_2			= pow(12.0*Omega_m*h*h, 0.424)*(1+pow(45.0*Omega_m*h*h, -0.582));
	double alpha_c		= pow(a_1, -Omega_b/Omega_m)*pow(a_2, -pow(Omega_b/Omega_m, 3));
	...
	double T_c			= f*T_0_tilde(k, 1, beta_c) + (1-f)*T_0_tilde(k, alpha_c, beta_c);

	//Baryon transfer function
	double beta_node	= 8.41*pow(Omega_m*h*h, 0.435);
	double s_tilde		= s/pow(1+pow(beta_node/(k*s), 3), 0.33333); //phenomenological sound horizon
	double alpha_b		= 2.07*k_eq*s*pow(1+R_d, -0.75)*G((1+z_eq)/(1+z_d));
	double beta_b		= 0.5 + Omega_b/Omega_m + (3-2*Omega_b/Omega_m)*sqrt(pow(17.2*Omega_m*h*h, 2)+1);
	double T_b			= (T_0_tilde(k,1,1)/(1+pow(k*s/5.2, 2)) + alpha_b/(1+pow(beta_b/(k*s), 3))*exp(-pow(k/k_silk, 1.4)))*j_0(k*s_tilde);
...
//Time in Gyr
double cosmic_time(double z) { //valid for flat LCDM (source: arXiv:gr-qc/0606038v1)
	double conversion_factor = 978.5706; //(km/s/Mpc)^-1 to Gyr
	return (2/H_0)/(3*sqrt(Omega_L))*asinh(sqrt(Omega_L/Omega_m)/pow(z+1,1.5))*conversion_factor;
}

//The scale factor in any cosmology, by definition
double a_scale_factor_of_z(double z) {
	return 1.0/(1+z);
}
...
//Approximation for the scale factor in flat LCDM (https://physics.stackexchange.com/q/263725)
double a_scale_factor(double t) { //time in Gyr
	double conversion_factor = 978.5706; //(km/s/Mpc)^-1 to Gyr
	return pow(Omega_m/Omega_L * pow(sinh(1.5*sqrt(Omega_L)*H_0*t/conversion_factor),2), 0.3333);
}
//Approximation for flat LCDM (same source)
double coth(double x) { return cosh(x)/sinh(x); }
double H_hubble(double t) { //time in Gyr
	double conversion_factor = 978.5706; //(km/s/Mpc)^-1 to Gyr
	double Hubble = sqrt(Omega_L)*H_0*coth(1.5*sqrt(Omega_L)*H_0*t/conversion_factor); //in units of (km/s/Mpc)^-1
	return Hubble/conversion_factor; //in units of Gyr^-1
}
...
double D_growth_factor(double z) {
	...
	double D = 2.5/(1+z)*Omega_m_of_z/(pow(Omega_m_of_z,4./7)-Omega_L_of_z+(1+Omega_m_of_z/2)*(1+Omega_L_of_z/70));
	return D;
}
...
double logarithmic_derivative_f_1(double z) {
	...
	return pow(Omega_m_of_z, 4.0/7.0) + (1+Omega_m_of_z/2)*Omega_L_of_z/70;
}
...
double kick_step_size(double z1, double z2) {
	...
	//See appendix A of Quinn et al. (1997) astro-ph/9710043 for the kick operator K(t).
	// double conversion_factor = 978.5706; //(km/s/Mpc)^-1 to Gyr
	// return 2.0/(H_0/conversion_factor) * (sqrt(a_scale_factor_of_z(z2)) - sqrt(a_scale_factor_of_z(z1)));
}
...
double drift_step_size(double z1, double z2) {
	...
	//Below is an analytical value, but I think it may be incorrect
	//See appendix A of Quinn et al. (1997) astro-ph/9710043 for the drift operator D(t).
	// double conversion_factor = 978.5706; //(km/s/Mpc)^-1 to Gyr
	// return 2.0/(H_0/conversion_factor) * (1.0/sqrt(a_scale_factor_of_z(z1)) - 1.0/sqrt(a_scale_factor_of_z(z2)));
}
...
inline void test_cosmology(std::string fname) {
	...
	while (z_rs >= z_penultimate) {
		...
		//Output a row
		of << t << " " << z_rs << " " << a1 << " " << a2 << " " << H1 << " " << H2 << " " << D << " " << f1 << " " << f2 << std::endl;
	}
	...
	if (Omega_total!=1) {
		std::cout << "Warning: Omega != 1, while this is assumed for some methods." << std::endl;
	}
}
