...
# Computes the analytical solution of the 3D Noh test.
...
rho_s = np.where(r < rs,
                 np.ones(N) * rho0 * ((gas_gamma + 1.) / (gas_gamma - 1.))**3,
                 rho0 * (1. + v0 * time / r)**2)
v_s = np.where(r < rs, np.zeros(N), -np.ones(N) * v0)
P_s = np.where(r < rs,
               np.ones(N) * 0.5 * rho0 * v0**2 * \
                 (gas_gamma + 1.)**3 / (gas_gamma - 1.)**2,
               np.ones(N) * P0)
...
rhorange = np.where(rrange < rs,
                    np.ones(1000) * rho0 * \
                      ((gas_gamma + 1.) / (gas_gamma - 1.))**3,
                    np.ones(1000) * rho0 * (1. + v0 * time / rrange)**2)
