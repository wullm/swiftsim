...
# Read glass pre-ICs
infile  = h5py.File(glass_name, "r")
one_glass_pos = infile["/PartType0/Coordinates"][:,:]
one_glass_h   = infile["/PartType0/SmoothingLength"][:]

# Make copies
one_glass_pos /= num_copy
one_glass_h /= num_copy

# x direction
pos_all = one_glass_pos
h_all = one_glass_h
for i in range(1, num_copy):
  pos_all = append(pos_all, one_glass_pos + array([i * 1. / num_copy, 0., 0.]),
                   axis=0)
  h_all = append(h_all, one_glass_h)

# y direction
one_glass_pos = pos_all
one_glass_h = h_all
for i in range(1, num_copy):
  pos_all = append(pos_all, one_glass_pos + array([0., i * 1. / num_copy, 0.]),
                   axis=0)
  h_all = append(h_all, one_glass_h)
...
one_glass_h = h_all
for i in range(1, num_copy):
  pos_all = append(pos_all, one_glass_pos + array([0., 0., i * 1. / num_copy]),
                   axis=0)
  h_all = append(h_all, one_glass_h)
